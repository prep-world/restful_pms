...
import { useMyBookings, BookingStatus, canCancelBooking, getBookingStatusColor } from '@/hooks/useBooking';
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from '@/components/ui/card';
import { Button } from '@/components/ui/button';
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from '@/components/ui/select';
import { Badge } from '@/components/ui/badge';
import { Skeleton } from '@/components/ui/skeleton';
import { Calendar, Car, Clock, ParkingSquare, MapPin, Filter } from 'lucide-react';
import Link from 'next/link';
import { useCancelBooking } from '@/hooks/useBooking';

const BookingsPage = () => {
  const { user } = useAuthCheck();
  const { data: bookingsData, isLoading: bookingsLoading, error } = useMyBookings();
  ...
  return (
    <div className="container mx-auto p-4">
      ...
      <p className="text-gray-500 mb-8">View and manage your parking bookings</p>
                  ...
                  <div className="flex items-start space-x-3">
                    ...
                    <div>
                      ...
                      <div className="flex gap-2 mt-1">
                        <Badge variant="secondary">{booking.vehicle.type}</Badge>
                        <Badge
                          className={`bg-${getBookingStatusColor(booking.status)}-100 text-${getBookingStatusColor(booking.status)}-800`}
                        >
                          {booking.status}
                        </Badge>
                      </div>
