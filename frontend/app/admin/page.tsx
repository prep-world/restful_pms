      ...
      >
        <div className="p-4 border-b flex items-center justify-between">
          <div className={`flex items-center ${!isSidebarOpen && 'justify-center w-full'}`}>
            <div className="rounded-full bg-gradient-to-br from-blue-500 to-indigo-600 p-2 shadow">
              <Car className="h-5 w-5 text-white" />
            </div>
            {isSidebarOpen && (
              <h2 className="ml-2 font-bold text-lg">FleetFlow</h2>
            )}
          </div>
          <button
            onClick={() => setIsSidebarOpen(!isSidebarOpen)}
            className={`${!isSidebarOpen && 'hidden'} text-gray-500 hover:text-gray-700`}
          >
            {isSidebarOpen ? (
              <X className="h-5 w-5" />
            ) : (
        ...
        <div className="flex-grow overflow-y-auto">
          <div className="p-4">
            {isSidebarOpen ? (
              <div className="flex flex-col items-center mb-6 p-4 bg-gradient-to-br from-blue-600 to-indigo-700 rounded-lg text-white">
                <User className="h-12 w-12 mb-2" />
                <h3 className="font-semibold text-lg">{user?.firstName} {user?.lastName}</h3>
                <p className="text-xs opacity-80">{user?.email}</p>
                <Badge className="mt-2 bg-white/20 hover:bg-white/30 border-none text-white">
                  {user?.role}
                </Badge>
              </div>
            ) : (
            ...
            <div className="space-y-2">
              <button
                className={`w-full ${
                  isSidebarOpen ? 'flex items-center px-4 py-3' : 'flex flex-col items-center justify-center py-3'
                } rounded-lg bg-blue-50 text-blue-600 hover:bg-blue-100 transition-colors`}
              >
                <Calendar className={`${isSidebarOpen ? 'mr-3' : 'mb-1'} h-5 w-5`} />
                {isSidebarOpen ? 'Bookings' : <span className="text-xs">Bookings</span>}
              </button>
              <button
                className={`w-full ${
                  isSidebarOpen ? 'flex items-center px-4 py-3' : 'flex flex-col items-center justify-center py-3'
                } rounded-lg hover:bg-blue-50 text-gray-600 hover:text-blue-600 transition-colors`}
              >
                <Layers className={`${isSidebarOpen ? 'mr-3' : 'mb-1'} h-5 w-5`} />
                {isSidebarOpen ? 'Parking Slots' : <span className="text-xs">Slots</span>}
              </button>
              <button
                className={`w-full ${
                  isSidebarOpen ? 'flex items-center px-4 py-3' : 'flex flex-col items-center justify-center py-3'
              ...
              >
                <Car className={`${isSidebarOpen ? 'mr-3' : 'mb-1'} h-5 w-5`} />
                {isSidebarOpen ? 'Vehicles' : <span className="text-xs">Vehicles</span>}
              </button>
              <button
                className={`w-full ${
                  isSidebarOpen ? 'flex items-center px-4 py-3' : 'flex flex-col items-center justify-center py-3'
              ...
              >
                <User className={`${isSidebarOpen ? 'mr-3' : 'mb-1'} h-5 w-5`} />
                {isSidebarOpen ? 'Users' : <span className="text-xs">Users</span>}
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="p-4 border-t">
          <button
            ...
            className={`w-full ${
              isSidebarOpen ? 'flex items-center justify-center' : 'flex flex-col items-center justify-center'
            } px-4 py-2 rounded-lg bg-red-50 text-red-600 hover:bg-red-100 transition-colors`}
          >
            <LogOut className={`${isSidebarOpen ? 'mr-2' : 'mb-1'} h-5 w-5`} />
            {isSidebarOpen ? 'Logout' : <span className="text-xs">Logout</span>}
          </button>
        </div>
      </div>
